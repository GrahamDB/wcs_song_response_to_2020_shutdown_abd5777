...
print.table(apply(with(vp_obs,table(bird=bird,site=location,year=year)) >0,c(2:3),sum), zero.print=".")
print.table(apply(with(vp_obs,table(bird=bird,region=region,year=year)) >0,c(2:3),sum), zero.print="XXX")

cat("\n======   \n\nRun models with just 2016 and 2020 data:\n")
song_reduced_obs <- vp_obs %>% filter(year %in% c("Y2020","Y2016"))

me_vp_model_set_re_dialect <- function(vp.data=vp_obs){
  me_A <- list()
